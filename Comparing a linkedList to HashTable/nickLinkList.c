...
* sorted linked list and display if the item was found      *
* also there is a small function that takes out the newline *
...
/*****************************************************************
                                                       *
 * This function will print the results found   to the screen.   *
 *                                                               *
                                        *
 *****************************************************************/
void showSearchResultsForLinkList(NICK_NAMES *head,char buffer[],int* linkListCount)
{
	...
	while( item != NULL )
	{
		counter++;
		*linkListCount+= kTrue;
		if(strcmp(buffer,item->kNames)==0)
		{
			nameWasFound++;
			break;
			
		}
		else if(strcmp(item->kNames,buffer) >= 0)
		{
			//item not found and so we dont go at the end of the list we break  
						break;
		}
	...
	if(nameWasFound == kTrue)
	{
		printf("%s was found in the linked list in %d comparisons\n",buffer,counter);
	}
	else
	{
		printf("%s was not found in the linked list in after %d comparisons\n",buffer,counter);
	}
